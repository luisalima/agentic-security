...
def _(mo):
    ...
    This is the lowest-hanging fruit in agentic security:
    you can apply it to off-the-shelf agents you don't control.

    ## The Core Principle

    > **Assume the agent will be compromised. Limit the damage.**

    Instead of trusting the agent to behave correctly,
    constrain its environment so that even malicious behavior can't cause catastrophic harm.
    """)
    return
...
def _(mo):
    ...
    You can containerize, sandbox, and restrict any agent — even one you downloaded
    as a binary. Software defenses like dual LLM or dry-run evaluation require
    modifying the agent's code.

    **Start here. Add software defenses later.**
    """)
    return
...
def _(mo):
    ...
    For MCP servers, only connect the servers relevant to the task.
    Review each tool's permissions before granting access.
...
def _(mo):
    ...
    For those, you need the software-level defenses in the next section.
    """)
    return
